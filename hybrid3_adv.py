# -*- coding: utf-8 -*-
# 2-D (Lx,Ly) PINN/regression comparison with gradient-norm adaptive loss weighting.
# Two modes, same eval (resonance-region held-out circular phase error):
#   adv = Maxwell residual + data, data term rescaled to match the physics term's gradient norm
#   reg = pure data-driven regression on the same scarce anchors
# h=1mm fixed. Output: ablation3_adv.npz.
import os
...
def run_mode(MODE):
    ...
    for it in range(1, N_ITERS+1):
        ...
        l_phys = l_pde+50*l_patch+30*l_gnd+5*l_top+l_wall
        if MODE == 'hybrid':
            ...
            if it == 1 or it % 200 == 0:
                ...
                def _gn(L):
                    gs = torch.autograd.grad(L, _ps, retain_graph=True, allow_unused=True)
                    return torch.sqrt(sum((g.detach()**2).sum() for g in gs if g is not None) + 1e-30)
                adv_lam = float((_gn(l_phys)/_gn(ld)).clamp(0.1, 1e4))
            loss = loss + l_phys + adv_lam*ld
        else:
            loss = loss + l_phys
        ...
        if it % max(1, N_ITERS//8) == 0 or it == 1:
            ...
            if MODE in ('hybrid', 'adv'): msg += f" | data {ld.item():.2e}"
            if MODE == 'adv': msg += f" | lam {adv_lam:.2e}"
...
print("\n===== ADVANCED-HYBRID (NTK/grad-norm weighting) vs PURE DATA (2-param, 12 anchors) =====", flush=True)
for MODE in ('adv', 'reg'):
    print(f"  {MODE:7s}: {results[MODE][1]:.2f} deg  (overall {results[MODE][0]:.2f})", flush=True)
print(f"\n  even an ADVANCED (gradient-norm-balanced) hybrid: adv {results['adv'][1]:.2f} vs pure-data reg "
      f"{results['reg'][1]:.2f} -> "
      + ("physics STILL HURTS (reg better)" if results['reg'][1] < results['adv'][1] else "physics helps"), flush=True)
np.savez(os.path.join(DIR, 'ablation3_adv.npz'),
         N_anch=N_ANCH, adv_res=results['adv'][1], reg_res=results['reg'][1],
         adv_ov=results['adv'][0], reg_ov=results['reg'][0])
print("saved ablation3_adv.npz", flush=True)
