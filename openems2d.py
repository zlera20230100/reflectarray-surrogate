...
# Lx (transverse width) and Ly (resonant length, E along y). Waveguide simulator (PMC at +-x, PEC at +-y,
# normal incidence), grounded FR4, 24 GHz, two-plane traveling-wave fit Gamma=A/B at z1=5, z2=6.5 mm.
# Sweeps Ly in linspace(2,6,11) x Lx in linspace(2,6,9) = 99 sims and saves ref2d.npz.
...
for iy, Ly in enumerate(Ly_s):
    ...
    for ix, Lx in enumerate(Lx_s):
        try:
            G,e1,e2 = run(rf"D:\openEMS_pkg\sim_uc2d_{sidx}", float(Lx), float(Ly))
...
np.savez(r"D:\实践三号“延安”\论文\ref2d.npz",
