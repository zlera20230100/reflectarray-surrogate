# -*- coding: utf-8 -*-
# openEMS unit-cell reflection of a dual-strip reflectarray cell (two separated y-dipoles of lengths
# Lx and Ly), 2D geometry sweep, E along y. Waveguide simulator (PMC at +-x, PEC at +-y, normal
# incidence), grounded FR4, 24 GHz, two-plane traveling-wave fit Gamma=A/B at z1=5, z2=6.5 mm.
# Sweeps Ly in linspace(2,6,11) x Lx in linspace(2,6,9) = 99 sims and saves ref2d_dual.npz.
...
WSTRIP = 0.5; XOFF = 1.6   # dual-resonance: two separated y-dipoles
...
def run(sim, Lx, Ly):
    ...
    dm=CSX.AddMetal('dual')
    dm.AddBox([-XOFF-WSTRIP/2,-Lx/2,h],[-XOFF+WSTRIP/2,Lx/2,h],priority=10)   # strip A, length Lx
    dm.AddBox([ XOFF-WSTRIP/2,-Ly/2,h],[ XOFF+WSTRIP/2,Ly/2,h],priority=10)   # strip B, length Ly
    ...
    g.AddLine('x', dedupe([-p/2,-XOFF-WSTRIP/2,-XOFF+WSTRIP/2,XOFF-WSTRIP/2,XOFF+WSTRIP/2,p/2]))
    g.AddLine('y', dedupe([-p/2,-Lx/2,Lx/2,-Ly/2,Ly/2,p/2]))
...
for iy, Ly in enumerate(Ly_s):
    ...
    for ix, Lx in enumerate(Lx_s):
        try:
            G,e1,e2 = run(rf"D:\openEMS_pkg\sim_uc2dD_{sidx}", float(Lx), float(Ly))
...
np.savez(r"D:\实践三号“延安”\论文\ref2d_dual.npz",
