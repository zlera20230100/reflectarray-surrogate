# -*- coding: utf-8 -*-
# openEMS unit-cell reflection of a cross-dipole reflectarray cell, 2D geometry sweep over Lx (x-arm)
# and Ly (resonant y-arm, E along y). Waveguide simulator (PMC at +-x, PEC at +-y, normal incidence),
# grounded FR4, 24 GHz, two-plane traveling-wave fit Gamma=A/B at z1=5, z2=6.5 mm.
# Sweeps Ly in linspace(2,6,11) x Lx in linspace(2,6,9) = 99 sims and saves ref2d_cross.npz.
...
WSTRIP = 0.5   # cross-dipole arm width (mm)
...
def run(sim, Lx, Ly):
    ...
    cm=CSX.AddMetal('cross'); cm.AddBox([-Lx/2,-WSTRIP/2,h],[Lx/2,WSTRIP/2,h],priority=10)   # x-arm
    cm.AddBox([-WSTRIP/2,-Ly/2,h],[WSTRIP/2,Ly/2,h],priority=10)   # y-arm (E||y -> Ly is resonant arm)
    ...
    g.AddLine('x', dedupe([-p/2,-Lx/2,Lx/2,-WSTRIP/2,WSTRIP/2,p/2]))
    g.AddLine('y', dedupe([-p/2,-Ly/2,Ly/2,-WSTRIP/2,WSTRIP/2,p/2]))
...
for iy, Ly in enumerate(Ly_s):
    ...
    for ix, Lx in enumerate(Lx_s):
        try:
            G,e1,e2 = run(rf"D:\openEMS_pkg\sim_uc2dX_{sidx}", float(Lx), float(Ly))
...
np.savez(r"D:\实践三号“延安”\论文\ref2d_cross.npz",
